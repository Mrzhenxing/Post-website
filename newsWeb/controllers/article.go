...
func(this*ArticleController)ShowIndex(){
	...
	if err!=nil{
		beego.Error("redis连接数据失败",err)
		return
	}
	defer conn.Close()
	//获取数据 如果娶不到数据不会报err  get一个空  借口为nil
	...
	//从mysql中获取数据  把数据存到redis里面
	//conn,err:=redis.Dial("tcp",":6379")
	//if err!=nil{
	//	beego.Info("redis连接数据失败",err)
	//	return
	//}
	//
	//
	//resp,err:=conn.Do("set","newsWeb")
	////序列化   定义容器
	//var buffer bytes.Buffer
	////定义编码器
	//enc:=gob.NewEncoder(&buffer)
